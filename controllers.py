...
class VirtualNeuroMuscularController(Controller):
    ...
    def __init__(self, exo: Exo, hardware_factor, config,
                 Kp: int = 40,
                 Ki: int = 400,
                 Kd: int = 0,
                 ff: int =120):
        ...
        super().__init__(exo)
        self.exo = exo
        self.hardware_factor = hardware_factor
        self.mscl = None
        self.config = config

        self.name_0, self.F_mtu_0, self.l_ce_0, self.v_ce_0 = 0, 0, 0, 0

        self.n_update_muscle = 3
    ...
    def command(self, reset):
        ...
        m_stim = 0.01 + self.config.VNMC_GAIN * F_mtu_0

        self.update_muscle_model(m_stim, self.phi0)

        # self.name_0, self.F_mtu_0, self.l_ce_0, self.v_ce_0 = self.mscl.getStates()
        # trq = self.mscl.getTorque()

        self.exo.command_torque(self.hardware_factor * self.trq)
        # print('Torque commanded: ', self.hardware_factor * self.trq)

    def update_muscle_model(self, m_stim, phi0):

        self.mscl.update(m_stim, math.radians(phi0))

        self.name_0, self.F_mtu_0, self.l_ce_0, self.v_ce_0 = self.mscl.getStates()
        self.trq = self.mscl.getTorque()

        self.exo.data.mtu_force = self.F_mtu_0
        self.exo.data.length_CE = self.l_ce_0
        self.exo.data.velocity_CE = self.v_ce_0
        self.exo.data.vnmc_torque = self.trq
        self.exo.data.muscle_stimulation = m_stim
