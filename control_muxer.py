...
def get_gse_and_sm_lists(exo_list, config: Type[config_util.ConfigurableConstants]):
    ...
    if config.TASK == config_util.Task.WALKING:
        for exo in exo_list:
            ...
            reel_in_controller_used = constants.ControllerUsed.SmoothReelInController
            swing_controller = controllers.StalkController(
                exo=exo, desired_slack=config.SWING_SLACK)
            swing_controller_used = constants.ControllerUsed.StalkController
            if config.STANCE_CONTROL_STYLE == config_util.StanceCtrlStyle.FOURPOINTSPLINE:
                reel_out_controller = controllers.SoftReelOutController(
                    exo=exo, desired_slack=config.SWING_SLACK)
                reel_out_controller_used = constants.ControllerUsed.SoftReelOutController
                ...
                stance_controller_used = constants.ControllerUsed.FourPointSplineController
                ...
                stance_controller_used = constants.ControllerUsed.SawickiWickiController
            elif config.STANCE_CONTROL_STYLE == config_util.StanceCtrlStyle.VIRTUALNEUROMUSCULARCONTROLLER:
                reel_out_controller = controllers.SoftReelOutController(
                    exo=exo, desired_slack=8000, force_timer_to_complete=True)
                reel_out_controller_used = constants.ControllerUsed.SoftReelOutController
                stance_controller = controllers.VirtualNeuroMuscularController(exo=exo, hardware_factor=config.HARDWARE_FACTOR, config=config)
                stance_controller_used = constants.ControllerUsed.VirtualNeuromusuclarController
        ...
        for exo in exo_list:
            ...
            standing_controller_used = constants.ControllerUsed.GenericImpedanceController
            if config.STANCE_CONTROL_STYLE == config_util.StanceCtrlStyle.GENERICIMPEDANCE:
                ...
                slip_controller_used = constants.ControllerUsed.GenericImpedanceController
                ...
                slip_controller_used = constants.ControllerUsed.FourPointSplineController
                ...
                slip_controller = controllers.FourPointSplineController(
                    ...
                    use_gait_phase=False,
                    peak_hold_time=0.1)
                slip_controller_used = constants.ControllerUsed.FivePointSplineController
                slip_recovery_time = 0.99
