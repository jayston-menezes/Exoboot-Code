...
class Exo():
    ...
    class DataContainer:
        ...
        # Newly added Dataclass Variables
        controller: str = None
        ...
        commanded_ankle_impedance: float = None
        muscle_stimulation: float = 0
    ...
    def read_data(self, exo_boot_side=None, iteration_count=None, loop_time=None):
        ...
        if self.IS_HARDWARE_CONNECTED:
            ...
            if exo_boot_side == constants.Side.LEFT:
                actpack_data = self.left_side_offline_data
            else:
                actpack_data = self.right_side_offline_data
            ank_ang = actpack_data['ankle_angle'][iteration_count]
            ankle_angle_temp = ank_ang
            self.data.state_time = actpack_data['state_time'][iteration_count]
            self.data.ankle_angle = ankle_angle_temp
            self.data.temperature = actpack_data['temperature'][iteration_count]
            self.data.accel_x = actpack_data['accel_x'][iteration_count]
            self.data.accel_y = actpack_data['accel_y'][iteration_count]
            self.data.accel_z = actpack_data['accel_z'][iteration_count]
            self.data.gyro_x = actpack_data['gyro_x'][iteration_count]
            self.data.gyro_y = actpack_data['gyro_y'][iteration_count]
            self.data.gyro_z = actpack_data['gyro_z'][iteration_count]
            '''Motor angle and current are kept in Dephy's orientation, but ankle
            angle and torque are converted to positive = plantarflexion.'''
            self.data.motor_angle = actpack_data['motor_angle'][iteration_count]
            self.data.motor_velocity = actpack_data['motor_velocity'][iteration_count]
            self.data.motor_current = actpack_data['motor_current'][iteration_count]
            self.data.ankle_torque_from_current = actpack_data['ankle_torque_from_current'][iteration_count]
            try:
                self.data.status_ex = actpack_data['status_ex'][iteration_count]
                self.data.status_mn = actpack_data['status_mn'][iteration_count]
                self.data.status_re = actpack_data['status_re'][iteration_count]
            except:
                pass
        ...
        if self.IS_HARDWARE_CONNECTED:
            ...
            if self.has_calibrated:
                self.data.slack = float(actpack_data['slack'][iteration_count])
            if self.last_state_time is None or last_ankle_angle is None or self.data.state_time - self.last_state_time > 20:
                self.data.ankle_velocity = 0
            elif self.data.state_time == self.last_state_time:
                pass  # Keep old velocity
            else:
                self.data.ankle_velocity = float(actpack_data['ankle_velocity'][iteration_count])
    ...
    def standing_calibration_offline(self, calibration_mV: int = 1300):
        '''Brings up slack, calibrates ankle and motor offset angles.'''
        self.command_voltage(desired_mV=self.motor_sign * calibration_mV)
        self.has_calibrated = True
        self.motor_offset = (self.data.motor_angle -
                             self.ankle_angle_to_motor_angle(self.data.ankle_angle))
        for ramp_down_value in np.arange(1, 0, -0.01):
            self.command_voltage(desired_mV=ramp_down_value * self.motor_sign * calibration_mV)
